...
from models.move import Accumulator, MultiTurn
from pokemon.cache_manager import get_move_cache, save_move_cache, status_effect_to_dict
...
def convert_move(move_data: dict) -> Optional[dict]:
    ...
    is_api_multi_turn = (
        min_turns is not None and min_turns > 1 and move_name in MULTI_TURN_OVERRIDES
    )
    ...
    for entry in move_data.get("effect_entries", []):
        if entry.get("language", {}).get("name") == "en":
            ...
            if effect_chance and "$effect_chance" in description:
                description = description.replace("$effect_chance", str(effect_chance))
            break
