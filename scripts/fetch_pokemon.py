...
def fetch_and_cache_pokemon(name):
    ...
    formatted = {
        ...
        "abilities": [
            ...
            if int(a["ability"]["url"].split("/")[-2]) <= 10000
        ],  # filter invalid IDs
        "hp": next(s["base_stat"] for s in data["stats"] if s["stat"]["name"] == "hp"),
        "stat_attk": next(
            s["base_stat"] for s in data["stats"] if s["stat"]["name"] == "attack"
        ),
        "stat_def": next(
            s["base_stat"] for s in data["stats"] if s["stat"]["name"] == "defense"
        ),
        "stat_sp_attk": next(
            s["base_stat"]
            for s in data["stats"]
        ...
        "stat_spd": next(
            s["base_stat"] for s in data["stats"] if s["stat"]["name"] == "speed"
        ),
        "stat_acc": 1.0,
        "stat_eva": 1.0,
    }
